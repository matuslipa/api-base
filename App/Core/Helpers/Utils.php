...
final class Utils
{
    ...
    public static function createFileNameForStorageFromString(
        ...
        $filename = \str_pad(\array_pop($result), 2, '0', \STR_PAD_LEFT);
        ...
        $directories = \array_map(
            fn ($dir) => 'd' . \str_pad($dir, 2, '0', \STR_PAD_LEFT),
            $result
        );
    ...
    public static function urify(string $value, string $separator = '-'): string
    {
        ...
        $title = \preg_replace('![' . \preg_quote($separator, $separator) . '\s]+!u', $separator, $title);
