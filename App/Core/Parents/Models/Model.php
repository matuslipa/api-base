...
abstract class Model extends \Illuminate\Database\Eloquent\Model implements ModelInterface
{
    ...
    /**
     * @param int|string|string[]|int[] $key
    ...
    public function setAttribute($key, $value): self
    {
        ...
        foreach (static::$setAttributeMutators[static::class] ?? [] as $mutator) {
            ...
            /** @var true|false $interrupt can be modified by mutator using & ref */
            if ($interrupt) {
                return $this;
            }
        }
    ...
    {
        return $attribute === null ? ! empty($this->previous) : isset($this->previous[$attribute]);
    }
     ...
     *
     * @return \App\Core\Values\Diff\Diff<mixed>|null
     */
    ...
    public function loadSumFromDefinitions(array|RelationSumDefinition $relations): self
    {
        ...
        ModelCollection::make([$this])->loadSumFromDefinitions($relations);
    ...
    protected function getExtendedAttributesDiffUsingResolvers(
        ...
        return \array_reduce(
            ...
            fn (ModelAttributesDiff $carryDiff, \Closure $resolver): ModelAttributesDiff => $carryDiff->merge(
     ...
     *
     * @param string $key
     * @return string|\App\Core\Values\DynamicModelCast
     */
    protected function getCastType($key): DynamicModelCast|string
    {
        ...
        if ($type) {
            ...
            if (\is_string($type) && Utils::isClassName($type)) {
                ...
                // class names we shall convert to the custom unified cast type...
                return new DynamicModelCast($type);
            }
     ...
     * @inheritDoc
     */
    protected function resolveCasterClass($key): DynamicModelCast|string
    {
    ...
    protected function castAttribute($key, $value): mixed
    {
        ...
        return match ($castType) {
            CastTypesEnum::INT => (int) $value,
            CastTypesEnum::STRING => (string) $value,
            CastTypesEnum::BOOL => (bool) $value,
            CastTypesEnum::ARRAY => $this->fromJson($value),
            CastTypesEnum::DATE => $this->asDate($value)->toImmutable(),
            CastTypesEnum::DATETIME => $this->asDateTime($value)->toImmutable(),
            default => $value,
        };
    }
     ...
     *
     * @param string $alias
     * @param array<string|int,callable|string> $loader
     *
     * @return int
     */
